...
void AntSolution::constructGreedy(double alpha) {
...
void AntSolution::random_greedySelection(std::vector<std::pair<uint16_t, uint16_t>> &extended_candidate_list, double alpha) {
      ...
      //       if (score(current_pair.first, current_pair.second) < score(cVertex, cCluster)) {
      //             cVertex = current_pair.first;
      //             cCluster = current_pair.second;
      //       }
      // }

      std::vector<std::pair<double, int>> ranked_candidates;
      ranked_candidates.reserve(extended_candidate_list.size());
      randomShuffle(extended_candidate_list);

      for (int i = 0; i < extended_candidate_list.size(); ++i) {
            auto& p = extended_candidate_list[i];
            ranked_candidates.emplace_back(score(p.first, p.second), i);
      }

      std::sort(ranked_candidates.begin(), ranked_candidates.end(), std::greater<>());
      ...
      for (uint32_t i = 0; i < s; ++i) {
            weights.push_back(ranked_candidates[i].first);
      }

      std::discrete_distribution<int> dist(weights.begin(), weights.end());
      int local_idx = dist(rng[omp_get_thread_num()]);

      // Choose pair [vertex, cluster]
      int winner_idx = ranked_candidates[local_idx].second;
      auto [cVertex, cCluster] = extended_candidate_list[winner_idx];
...
void AntSolution::greedy_randomSelection(std::vector<std::pair<uint16_t, uint16_t>> &extended_candidate_list, double alpha) {
      ...
      for (auto it = extended_candidate_list.begin(); it != extended_candidate_list.end(); ++it) {
            if (score(it -> first, it -> second) + parameters.eps <= threshold) {
...
void AntSolution::constructRandomized() {
