...
std::tuple<double, uint16_t, uint16_t, uint16_t> AntSolution::exploreInsert(bool objective) {
...
bool AntSolution::isFeasibleInsert(uint16_t vPos, uint16_t source, uint16_t target) {
...
double AntSolution::evaluateInsert(bool objective, uint16_t vPos, uint16_t source, uint16_t target) {
      uint16_t vIndex = partitions[source][vPos];
      if (!objective) {
