...
void AntSolution::addVertex(uint16_t vIndex, uint16_t cIndex) {
      ...
      for (uint16_t v = 0; v < instance.nV; v++) {
            ...
            pheromoneSum[v][cIndex] += tau[v][vIndex];
      }
...
void AntSolution::insertVertex(uint16_t vPos, uint16_t source, uint16_t target) {
      ...
      for (uint16_t v = 0; v < instance.nV; v++) {
            ...
            pheromoneSum[v][source] -= tau[v][vIndex];
            pheromoneSum[v][target] += tau[v][vIndex];
      }
...
void AntSolution::exchangeVertex(uint16_t fPos, uint16_t fClus, uint16_t sPos, uint16_t sClus) {
      ...
      for (uint16_t v = 0; v < instance.nV; v++) {
            ...
            pheromoneSum[v][fClus] += tau[v][sVertex] - tau[v][fVertex];
            pheromoneSum[v][sClus] += tau[v][fVertex] - tau[v][sVertex];
      }
