...
Algorithm::Algorithm(const string& pathInstance, unsigned ne) {
    ...
    if(parameters.ALGlg > 0) {
        ...
        fileEvol.open("results/logs/legacy_logs/evolution/" + instance + "_" + to_string(ne) + ".txt");
        if (!fileEvol) {
            cout << "\n The fileEvol file can not be opened.";
            exit(1);
        }

        fileSolu.open("results/logs/legacy_logs/solutions/" + instance + "_" + to_string(ne) + ".txt");
        if (!fileSolu) {
            cout << "\n The fileSolu file can not be opened.";
            exit(1);
        }

        fileCost.open("results/logs/legacy_logs/objectives/" + instance + "_" + to_string(ne) + ".txt");
        if (!fileEvol) {
